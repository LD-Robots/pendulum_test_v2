...
# ───────────────────────────────────────────────────────────
# CONTROLLER GAINS ROW
# ───────────────────────────────────────────────────────────


def _ros2_param_list(ctrl_name, timeout=5):
    """Return the list of fully-qualified parameter names for a controller."""
    try:
        result = subprocess.run(
            ["ros2", "param", "list", ctrl_name],
...
def _ros2_param_get(ctrl_name, param, timeout=3):
    """Return the param's value parsed from `ros2 param get`, or None."""
    try:
        result = subprocess.run(
            ["ros2", "param", "get", ctrl_name, param],
            capture_output=True, text=True, timeout=timeout,
        )
        if result.returncode != 0:
            return None
        # Output forms: "Double value is: 22.95" / "Boolean value is: True" / "Integer value is: 5"
        m = re.search(r"value is:\s*(\S+)", result.stdout)
        if not m:
            return None
        raw = m.group(1)
...
def _ros2_param_set(ctrl_name, param, value, timeout=3):
    """Set a parameter on a controller. Returns (ok, message)."""
    ...
    try:
        result = subprocess.run(
            ["ros2", "param", "set", ctrl_name, param, v_str],
            capture_output=True, text=True, timeout=timeout,
        )
        ok = result.returncode == 0 and "successful" in (result.stdout + result.stderr).lower()
        return ok, (result.stdout + result.stderr).strip()
    except (subprocess.TimeoutExpired, FileNotFoundError) as e:
        return False, str(e)


class ControllerGainsWidget(QWidget):
    """Per-controller gain editor.

    Auto-detects the parameter scheme:
      • JTC-style (``gains.<joint>.p`` / ``.d`` / ``.ff_acceleration_scale``)
        → Kp, Kd, Tff numeric inputs.
      • Plugin-style (top-level ``Kp`` / ``Kd``, optional ``ff_gravity``)
        → Kp, Kd numeric inputs plus a Gravity-comp checkbox if available.

    Values are pushed via ``ros2 param set`` on Apply.
    """

    log_signal = pyqtSignal(str, str)  # (message, color)

    def __init__(self, ctrl_name, joints, parent=None):
        super().__init__(parent)
        ...
        label.setStyleSheet(f"color: {C_PEACH};")
        label.setFixedWidth(56)
        layout.addWidget(label)

        self.kp_spin = self._make_spin(0.0, 1000.0, 0.5)
        self.kd_spin = self._make_spin(0.0, 100.0, 0.1)
        self.tff_spin = self._make_spin(-1000.0, 1000.0, 0.05)
        self.grav_check = QCheckBox("Gravity comp")
        self.grav_check.setStyleSheet(f"color: {C_TEXT};")
        ...
        self._tff_label = QLabel("Tff")
        layout.addWidget(self._tff_label)
        layout.addWidget(self.tff_spin)
        layout.addWidget(self.grav_check)
    ...
    def _detect_and_load(self):
        """Detection runs in a background thread — subprocess calls are slow."""
        self._set_enabled(False)
        threading.Thread(target=self._detect_worker, daemon=True).start()

    def _set_enabled(self, on):
        for w in (self.kp_spin, self.kd_spin, self.tff_spin,
                  self.grav_check, self.apply_btn):
            w.setEnabled(on)

    def _detect_worker(self):
        params = _ros2_param_list(self.ctrl_name)
        # JTC-style: gains.<joint>.p
        first_joint = self.joints[0] if self.joints else None
        jtc_kp = f"gains.{first_joint}.p" if first_joint else None
        if jtc_kp and jtc_kp in params:
            self._kp_param = jtc_kp
            self._kd_param = f"gains.{first_joint}.d"
            tff_candidate = f"gains.{first_joint}.ff_acceleration_scale"
            self._tff_param = tff_candidate if tff_candidate in params else None
            show_tff, show_grav = self._tff_param is not None, False
        elif "Kp" in params:
            self._kp_param = "Kp"
            self._kd_param = "Kd" if "Kd" in params else None
            self._grav_param = "ff_gravity" if "ff_gravity" in params else None
            show_tff, show_grav = False, self._grav_param is not None
        else:
            # Unknown scheme — hide everything but leave the row in place
            QTimer.singleShot(0, lambda: self._on_unknown_scheme())
            return

        kp_val = _ros2_param_get(self.ctrl_name, self._kp_param) if self._kp_param else None
        kd_val = _ros2_param_get(self.ctrl_name, self._kd_param) if self._kd_param else None
        tff_val = _ros2_param_get(self.ctrl_name, self._tff_param) if self._tff_param else None
        grav_val = _ros2_param_get(self.ctrl_name, self._grav_param) if self._grav_param else None

        def apply():
            self._tff_label.setVisible(show_tff)
            self.tff_spin.setVisible(show_tff)
            self.grav_check.setVisible(show_grav)
            if isinstance(kp_val, (int, float)):
                self.kp_spin.setValue(float(kp_val))
            if isinstance(kd_val, (int, float)):
                self.kd_spin.setValue(float(kd_val))
            if isinstance(tff_val, (int, float)):
                self.tff_spin.setValue(float(tff_val))
            if isinstance(grav_val, bool):
                self.grav_check.setChecked(grav_val)
            self._set_enabled(True)

        QTimer.singleShot(0, apply)

    def _on_unknown_scheme(self):
        self.kp_spin.setVisible(False)
        self.kd_spin.setVisible(False)
        self.tff_spin.setVisible(False)
        self._tff_label.setVisible(False)
        self.grav_check.setVisible(False)
        self.apply_btn.setText("(no gains)")
    ...
    def _on_apply(self):
        ...
        targets = []
        if self._kp_param:
            targets.append((self._kp_param, self.kp_spin.value()))
        if self._kd_param:
            targets.append((self._kd_param, self.kd_spin.value()))
        if self._tff_param and self.tff_spin.isVisible():
            targets.append((self._tff_param, self.tff_spin.value()))
        if self._grav_param and self.grav_check.isVisible():
            targets.append((self._grav_param, self.grav_check.isChecked()))
        ...
        def worker():
            ...
            for name, val in targets:
                ok, msg = _ros2_param_set(ctrl, name, val)
                results.append((name, val, ok, msg))

            def report():
                for name, val, ok, msg in results:
                    color = C_GREEN if ok else C_RED
                    self.log_signal.emit(
                        f"{ctrl}: {name} ← {val}" + ("" if ok else f"  [{msg}]"),
                        color,
                    )
...
class ActionPositionSenderWindow(QMainWindow):
    ...
    def _rebuild_joint_panel(self, selected_controllers):
        ...
        for ctrl_name in selected_controllers:
            ...
            # Per-controller gain editor (Kp / Kd / Tff or Gravity comp)
            gains_widget = ControllerGainsWidget(ctrl_name, joints)
            gains_widget.log_signal.connect(self._log)
            self._scroll_layout.addWidget(gains_widget)
